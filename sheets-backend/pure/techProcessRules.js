/**
 * Pure tech-process / catalog rules (no SpreadsheetApp / Drive).
 * Source of truth for Node tests. Synced into tech_process/TechProcessRules.gs
 * by scripts/sync-pure-rules.mjs (strip export keywords).
 */

export const ROMAN_ORDERS = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X'];
export const CATALOG_TYPES = ['tool', 'plate', 'jaw'];

export function setupOrderLabel_(order) {
  var n = Number(order);
...
export function isValidCatalogType_(type) {
 ...
 */
export function validateCatalogItemType_(actualType, expectedType, fieldName) {
 ...
 */
export function validateReorderIds_(orderedIds, existingIds) {
 ...
 */
export function planTechProcessCascadeDelete_(tp, setups, operations) {
...
export function nextSetupOrder_(setups) {
  var nextOrder = 0;
...
export function nextOperationOrder_(operations) {
  var nextOrder = 0;
